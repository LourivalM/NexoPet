...
@Component({
  selector: 'app-register-ong-form',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, LgpdTermsModalComponent, PhoneMaskDirective, CnpjMaskDirective],
  templateUrl: './register-ong-form.html',
  styleUrl: './register-ong-form.css'
})
export class RegisterOngFormComponent implements OnInit {
  ...
  onSubmit(): void {
    if (this.registerForm.valid) {
      console.log('Formulário de ONG Válido:', this.registerForm.value);
      // Enviar os dados para a API
      this.http.post('https://backend-api-tcc.onrender.com/register/ong', this.registerForm.value).subscribe({
        next: (response) => {
          console.log('Cadastro de ONG bem-sucedido:', response);
      ...
      console.log('Formulário de ONG Inválido');
